...
工作区（Workspace）：在一个 VS Code 窗口中打开的一个或多个文件夹及其相关配置的集合。

* **多项目聚合**：你可以将多个毫不相干的文件夹同时加入同一个工作区，方便同时开发和搜索。

* **专属项目配置**：工作区特有的 `.code-workspace` 配置文件，能让你为当前项目设置特定的代码风格、快捷键或禁用/启用某些插件，且**不会影响**全局设置。

* **状态保持**：它能记住你上次打开的文件、窗口布局、搜索历史等，下次只需双击工作区文件就能一键还原。

工作区缓存位于：`~/.config/Code/User/workspaceStorage` 目录。

#### 配置

在 VS Code 中确保 [Maven](../Java/Maven/Maven_Note.md) 项目创建及终端进程在‌**指定空工作区根目录**‌（而非 `${userHome}`）启动，核心是‌显式打开文件夹作为工作区，并配置 ‌`terminal.integrated.cwd` 为 ‌`${workspaceFolder}`。

---

## <span id="vscode_install">安装</span>

如果在 [Linux](../Linux/Linux_Note.md) 下使用压缩包解压安装方式安装 [VSCodium](Editors_Note.md#VSCodium)，需要编写相应的 Desktop 文件：
...
> [!tip] 
>  
>  `Icon` 这个属性就是配置 Logo 图标的路径的。
>  
> 默认 VSCode 或 VSCodium 的 Logo 图标放在安装目录下这个位置：`resources/app/resources/linux/code.png`，可以根本自己需要更换不同图标及图标位置。
...
> 
> ```shell
> jq: error (at /home/silascript/.config/Code/User/globalStorage/storage.json:68): Cannot iterate over null (null)
> ```
> 
> 所以最好在查找具体 Profile 之前，先查一次 `useraDataProfiles` 节点是不是存在：
> 
> 示例：
> 
> ```shell
...
语法：`code --profile profile名称 --list-extensions`

示例：

```shell
code --profile "Backend" --list-extensions
```

> [!info] 
> 
> CLI 文档： [The Visual Studio Code command-line interface](https://code.visualstudio.com/docs/editor/command-line)
